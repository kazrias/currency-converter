    ...
    <>
      {
        currentFour.filter(item => item !== undefined).length > 0 && (<div className={`block block-${num}`}>
          <ul className="currencies">
            ...
            <li onClick={() => setListIsActive(!listIsActive)} className='all-currencies'>
            ...
            <ul className={`hidden-currencies__list ${listIsActive ? 'active' : ''}`}>
              {
                hiddenList.map((cur,i)=>(
                  <li onClick={()=>onClickHiddenList(cur,i)} className='hidden-currencies__list-item' key={cur}>{cur}</li>
                ))
              }
            </ul>
          </div>
